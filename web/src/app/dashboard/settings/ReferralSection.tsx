...
export default function ReferralSection() {
  ...
  useEffect(() => {
    ...
    api.credits.transactions()
      .then(txs => {
        const referrals = txs.filter(tx => tx.reason?.toLowerCase().includes('referral') && tx.delta > 0)
        setCreditsEarned(referrals.reduce((sum, tx) => sum + tx.delta, 0))
  ...
  async function handleCopyMessage() {
    const message = t('referralShareMessage', { link })
  ...
  return (
    <div>
      ...
      >
        ...
        >
          {copiedLink ? t('referralCopyLinkCopied') : t('referralCopyLinkButton')}
        </button>
      </div>
      ...
      <div className="flex gap-2 mt-2">
        ...
        >
          {copiedMsg ? (
            t('referralCopyMessageCopied')
          ) : (
            <>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/>
              </svg>
              {t('referralCopyMessageButton')}
            </>
          )}
        </button>
      </div>
      ...
      <div className="flex items-center justify-between mt-2">
        <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
          {t('referralCodeLabel', { code })}
        </p>
        {creditsEarned > 0 && (
          <p className="text-xs font-medium" style={{ color: '#065f46' }}>
            {t('referralCreditsEarned', { creditsEarned, suffix: creditsEarned !== 1 ? 's' : '' })}
          </p>
        )}
      </div>
      <div className="flex items-center justify-between mt-1.5">
        <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
          {t('referralInvitesUsed', { count: referralCount, limit: BETA_INVITE_LIMIT })}
        </p>
        {referralCount >= BETA_INVITE_LIMIT && (
          <p className="text-xs font-medium" style={{ color: 'var(--color-text-muted)' }}>{t('referralLimitReached')}</p>
        )}
      </div>
    </div>
  )
